...
char**list=listLHA();
for(int n=0;list[n];n++) printf("%s\n",list[n]);

void *p=initpdfLHA("CT10",0);
printf("p=%p\n", p);

int maxNum, ID,pdg[20];
double xMin, xMax, qMin, qMax;
pdfInfoLHA(p, &maxNum, &ID,  &xMin,&xMax,&qMin,&qMax, pdg);
printf("ID=%d maxNum=%d xmin=%E xMax=%e qMin=%e qMax=%e\n", ID,maxNum,xMin,xMax,qMin,qMax);
for(int i=0;pdg[i];i++ ) printf(" %d ",pdg[i]);
printf("\n");

printf(" 22 in CT10 =%d   21 in CT10=%d \n",   pdgInLHA( p, 22),  pdgInLHA( p, 21) );
double x=0.1,q=81;

printf("alpha(%e)=%E   pdfFuncLHA(21,%e,%e)=%E\n",q,alphaLHA(p,q),x,q, pdfFuncLHA(p, 21,x,q));
