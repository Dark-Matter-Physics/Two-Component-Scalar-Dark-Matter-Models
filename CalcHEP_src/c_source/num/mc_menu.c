...
static int  in_setting(void)
{
  ...
  if(nin_int == 1)
  {
    ...
    {
      char strmen[] = "*"
      " Incomming  particle momentum[GeV] = PPP1      "
      " Incomming  particle helicity (not implemented)";

      Pcm=va_int[0];

      strmen[0]=strlen(strmen)/2;
      
      improveStr(strmen,"PPP1","%-10.4G",inPD);
...
*/      
      ...
      switch(mode)
      { case 0:   
           pinf_int(Nsub,1,mass,NULL);
           return returnCode;
        case 1: 
           correctDouble(50,12,"Enter new value ",&inPD,1);
