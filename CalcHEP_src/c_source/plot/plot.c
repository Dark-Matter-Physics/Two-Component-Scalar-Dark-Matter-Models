   ...
   { if(logX) 
     ...
     { if(logX)
       { fprintf(outfile,"c=sqrt(x%d[1]/x%d[0])\n",k,k);
         ...
         fprintf(outfile,"plt.errorbar(x%d, y%d[0:%d], dy%d[0:%d], xerr=[errL,errR], fmt='',label='%s')\n",k,k,Dim[k],k,Dim[k],Y[k]);
       }
       else     fprintf(outfile,"plt.errorbar(x%d, y%d[0:%d], dy%d[0:%d], xerr=%e, fmt='',label='%s')\n",k,k,Dim[k],k,Dim[k],0.45*(xMax-xMin)/Dim[k],Y[k]);
...
//                     writeMath1(baseF,xMin,xMax,dim,f[0],ff[0],title,xName,Y[0]);  
           ...
           case 0:  
             ...
             if(logY && (grafminmax.ymin <=0)) grafminmax.ymin=grafminmax.ymax*1E-10;
