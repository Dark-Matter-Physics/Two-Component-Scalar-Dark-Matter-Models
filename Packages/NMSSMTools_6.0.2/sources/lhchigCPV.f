      ...
      DO J=1,5
       ...
       IF(MH.GE.HM1(1).AND.MH.LT.HM1(N1))THEN
        ...
        IF(RW.LE.0.5d-2)THEN
         ...
         PROB(77)=PROB(77)+DDIM(DABS(CU(J))/LH,1d0)
...
* normalized to SM XS
      ...
      I=1
      DOWHILE(MA.GE.HAAMUS7(1,1) .AND. HAAMUS7(I,1).LE.MA
     .        .AND. I.LT.NHAAMUS7)
       I=I+1
      ENDDO
      IF(I.GT.1 .AND. MA.LE.HAAMUS7(NHAAMUS7,1))THEN
       LIMIT=HAAMUS7(I-1,2)+(MA-HAAMUS7(I-1,1))
     .         /(HAAMUS7(I,1)-HAAMUS7(I-1,1))
     .         *(HAAMUS7(I,2)-HAAMUS7(I-1,2))
       PROB(52)=PROB(52)+DDIM(C2BRHAA*BRMM(K)**2/LIMIT,1d0)
      ENDIF

      I=1
      DOWHILE(MA.GE.HAAMUS8(1,1) .AND. HAAMUS8(I,1).LE.MA
     .        .AND. I.LT.NHAAMUS8)
       I=I+1
      ENDDO
      IF(I.GT.1 .AND. MA.LE.HAAMUS8(NHAAMUS8,1))THEN
       LIMIT=HAAMUS8(I-1,2)+(MA-HAAMUS8(I-1,1))
     .         /(HAAMUS8(I,1)-HAAMUS8(I-1,1))
...
*     PROB(76) =/= 0: excluded 
      ...
      DOUBLE PRECISION M3(N3),L3(N3),M4(N4),L4(N4),MA,MH
      DOUBLE PRECISION MHC,XC(2,2),MH0(5),XH(5,5),MA2
      DOUBLE PRECISION CU(5),CUP(5),CD(5),CDP(5),CB(5),CBP(5),CJ(5),
     . CJP(5),CI(5),CG(5),CGP(5),CV(5),CZG(5),CZGP(5),CL(5),CLP(5)
      DOUBLE PRECISION BRJJ(5),BREE(5),BRMM(5),BRLL(5),
...
* ATLAS, arXiv:2004.01678
       ...
       IF(MA.GT.M1(1) .AND. MA.LE.M1(N1))THEN
        S=0d0
        J=1
        DOWHILE(M1(J).LT.MA)
         J=J+1
        ENDDO
        SMAX=L1(J-1)+(MA-M1(J-1))/(M1(J)-M1(J-1))*(L1(J)-L1(J-1))
        DO K=I+1,5
         MH=dsqrt(MH0(K))
         IF(MH.GE.MHMIN .AND. MH.LE.MHMAX)THEN
...
* CMS, arXiv:2111.01299
       ...
       IF(MA.GT.M3(1) .AND. MA.LE.M3(N3))THEN
        S=0d0
        J=1
        DOWHILE(M3(J).LT.MA)
         J=J+1
        ENDDO
        SMAX=L3(J-1)+(MA-M3(J-1))/(M3(J)-M3(J-1))*(L3(J)-L3(J-1))
        DO K=I+1,5
         MH=dsqrt(MH0(K))
         IF(MH.GE.MHMIN .AND. MH.LE.MHMAX)THEN
          S=S+(CJ(K)**2+CJP(K)**2)*BRHAZ(K,I)*BRMM(I)
         ENDIF
        ENDDO
        PROB(76)=PROB(76)+DDIM(S/SMAX,1d0)
       ENDIF

       IF(MA.GT.M4(1) .AND. MA.LE.M4(N4))THEN
        S=0d0
        J=1
        DOWHILE(M4(J).LT.MA)
         J=J+1
        ENDDO
        SMAX=L4(J-1)+(MA-M4(J-1))/(M4(J)-M4(J-1))*(L4(J)-L4(J-1))
        DO K=I+1,5
         MH=dsqrt(MH0(K))
         IF(MH.GE.MHMIN .AND. MH.LE.MHMAX)THEN
          S=S+(CJ(K)**2+CJP(K)**2)*BRHAZ(K,I)*BRMM(I)
