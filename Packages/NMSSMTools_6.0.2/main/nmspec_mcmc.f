	...
	SIGMU=DABS(PAR(4))/PAR(4)
      ...
      ELSE
       ...
       IF(M0DEV.EQ.0d0)THEN
        M0=M0CEN
       ELSE
 101    M0=M0CEN+MAX(DABS(M0CEN)*M0DEV,M0MIN)*GAU(IDUM)
       ...
       IF(M12DEV.EQ.0d0)THEN
        M12=M12CEN
       ELSE
 102    M12=M12CEN+MAX(DABS(M12CEN)*M12DEV,M12MIN)*GAU(IDUM)
       ...
       IF(A0DEV.EQ.0d0)THEN
        A0=A0CEN
       ELSE
        A0=A0CEN+MAX(DABS(A0CEN)*A0DEV,A0MIN)*GAU(IDUM)
       ENDIF

       IF(TBDEV.EQ.0d0)THEN
        PAR(3)=TBCEN
       ELSE
 103    PAR(3)=TBCEN+MAX(DABS(TBCEN)*TBDEV,TBMIN)*GAU(IDUM)
       ...
       IF(LDEV.EQ.0d0)THEN
        PAR(1)=LCEN
       ELSE
 104    PAR(1)=LCEN+MAX(DABS(LCEN)*LDEV,LMIN)*GAU(IDUM)
       ...
       ELSE
        IF(ALDEV.EQ.0d0)THEN
         ALINP=ALCEN
        ELSE
         ALINP=ALCEN+MAX(DABS(ALCEN)*ALDEV,ALMIN)*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       ELSE
        IF(AKDEV.EQ.0d0)THEN
         AKINP=AKCEN
        ELSE
         AKINP=AKCEN+MAX(DABS(AKCEN)*AKDEV,AKMIN)*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       ELSE
        IF(M1DEV.EQ.0d0)THEN
         M1INP=M1CEN
        ELSE
         M1INP=M1CEN+MAX(DABS(M1CEN)*M1DEV,M1MIN)*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       ELSE
        IF(M2DEV.EQ.0d0)THEN
         M2INP=M2CEN
        ELSE
         M2INP=M2CEN+MAX(DABS(M2CEN)*M2DEV,M2MIN)*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       ELSE
        IF(M3DEV.EQ.0d0)THEN
         M3INP=M3CEN
        ELSE
         M3INP=M3CEN+MAX(DABS(M3CEN)*M3DEV,M3MIN)*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       IF(MAFLAG.EQ.-1 .OR. MAFLAG.EQ.-2)THEN
        IF(XIFDEV.EQ.0d0)THEN
         XIFINP=XIFCEN
        ELSE
         XIFINP=XIFCEN+MAX(DABS(XIFCEN)*XIFDEV,XIFMIN)*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       IF(MAFLAG.EQ.-1 .OR. MAFLAG.EQ.-3)THEN
        IF(XISDEV.EQ.0d0)THEN
         XISINP=XISCEN
        ELSE
         XISINP=XISCEN+MAX(DABS(XISCEN)*XISDEV,XISMIN)*GAU(IDUM)
        ENDIF
       ENDIF
       ...
       IF(MAFLAG.EQ.-2 .OR. MAFLAG.EQ.-4)THEN
        ...
        ELSE
         IF(MSDEV.EQ.0d0)THEN
          MSINP=MSCEN
         ELSE
          MSINP=MSCEN+MAX(DABS(MSCEN)*MSDEV,MSMIN)*GAU(IDUM)
         ENDIF
        ENDIF
       ENDIF
       ...
       IF(MAFLAG.EQ.-5)THEN
        IF(MUDEV.EQ.0d0)THEN
         PAR(4)=MUCEN
        ELSE
         PAR(4)=MUCEN+MAX(DABS(MUCEN)*MUDEV,MUMIN)*GAU(IDUM)
        ENDIF
	IF(PAR(4).NE.0d0)SIGMU=DABS(PAR(4))/PAR(4)
       ...
       ELSE
        ...
        ELSE
         IF(MHDDEV.EQ.0d0)THEN
          MHDINP=MHDCEN
         ELSE
          MHDINP=MHDCEN+MAX(DABS(MHDCEN)*MHDDEV,MHDMIN)*GAU(IDUM)
         ENDIF
        ENDIF
        IF(MHUFLAG.EQ.0)THEN
         MHUINP=M0**2
        ELSE
         IF(MHUDEV.EQ.0d0)THEN
          MHUINP=MHUCEN
         ELSE
          MHUINP=MHUCEN+MAX(DABS(MHUCEN)*MHUDEV,MHUMIN)*GAU(IDUM)
         ENDIF
        ENDIF
       ENDIF
       ...
       IF(MUPDEV.EQ.0d0)THEN
        MUPINP=MUPCEN
       ELSE
        MUPINP=MUPCEN+MAX(DABS(MUPCEN)*MUPDEV,MUPMIN)*GAU(IDUM)
       ENDIF

       IF(MSPDEV.EQ.0d0)THEN
        MSPINP=MSPCEN
       ELSE
        MSPINP=MSPCEN+MAX(DABS(MSPCEN)*MSPDEV,MSPMIN)*GAU(IDUM)
       ENDIF

       IF(M3HDEV.EQ.0d0)THEN
        M3HINP=M3HCEN
       ELSE
        M3HINP=M3HCEN+MAX(DABS(M3HCEN)*M3HDEV,M3HMIN)*GAU(IDUM)
       ENDIF
