/* This file contains some tests of the 1-d integration routine "simpson" for some simple functions: exponentional (funExp),
   sin (funSin), Breit-Wigner(funBW), 1/x^p(funPole), a step function (funStep) and a random function (funRand).
   The code prints each function, the lower and upper limit of integration and the precision required.
   The result is compared with the precise analytical calculations, the relative error "err" and the number of calls are printed.
 */
