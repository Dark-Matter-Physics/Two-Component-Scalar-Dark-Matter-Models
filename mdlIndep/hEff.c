...
The function is presented in the end of file. It makes a sum of F(M/T,fermion) over all hadrons 
presented in "Monte Carlo particle numbering scheme" of PDG. If mode is 0,  then only meson 
contributions are contribute. Function F, for example, can be 
       F(M/T,fermion) = h1eff(M/T, fermion)  - the contribution of one 
dergee of freedom  to  entropy density.    

See below hEffIhadrons T) which calculate hEff  in ideal gas approximation taling into account SM bosons ( without gluon), 
leptons, and hadrons. 
...
static double I1T4(double t, int eta){ return 2*M_PI*M_PI/15*(g1eff(t,eta)-h1eff(t,eta));}            // e-m trace *T^4 for 1 degree of freedom,  t=M/T
...
static double hEffI(double T) // ideal gas approximation
{
  ...
  double sum= 2 +6*h1eff(80/T,1) + 3*h1eff(90/T,1) + h1eff(125/T,1)              // bosons
                +16                                                              // gluon
                +7./8.*2*3                                                       // neutrinos
                +7./8.*2*2 +4*h1eff(0.106/T,-1)  + 4*h1eff(1.7/T,-1)             // charged leptons
                +12*( 2*7./8.+  h1eff(0.101/T,-1)+ h1eff(1.4/T,-1) +h1eff(5/T,-1) + h1eff(175/T,-1)) // quarks
            ;
  return sum;
}

static double hEffIhadrons(double T) // ideal gas approximation
{

  double sum=  2 + 6*h1eff(80/T,1) + 3*h1eff(90/T,1) + h1eff(125/T,1)          // bosons without gluon
              +7./8.*2*3                                                       // neutrinos
              +7./8.*2*2 +4*h1eff(0.106/T,-1) + 4*h1eff(1.7/T,-1)              // charged leptons
              + hadrons(T, h1eff,1)                                            // hadrons
            ;
  return sum;
}
...
int main(int argc,char** argv)
{
  ...
  displayPlot("hEff: lattice and ideal gas approximation", "T", 0.1,0.7,0,3
               ,"hEff(lattice)", 0,hEff,NULL
               ,"hEffI(quarks)",0,hEffI,NULL
               ,"hEffI(hadrons)", 0, hEffIhadrons, NULL
             );   
