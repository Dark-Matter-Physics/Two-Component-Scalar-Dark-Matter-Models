...
int  LHAPDFList(void);
extern int  setLHAPDF(char *name, int num);
...
extern double cs23Vegas(numout * cc, int nsub, double Pcm, int ii3, double ptMin, 
    int nSess1, int nCall1,  int nSess2, int nCall2, 
...
extern  double darkOmegaInflDecay(double H0,double Gamma, double Beps,double *aEnd, double *Trh, int * isWimp,   int *err);
extern  double Ya(double a);
extern  double Za(double a);
extern  double ZaEq(double a);
extern  double Ta(double a);
extern  double Ha(double a);
extern  double dEq_a(double a);
extern  int    RHFlag;
extern  int InflInfo( double *Tmax, double *Trh, double *Teq,double*Tfo); 
...
extern double DwarfSignal(double vSigma, double *SpA);

extern double calcSpectrumPlus(char*proc22,int outP,double *spect, int*Err);
extern int   decaySpectrum(char*pName, int outP, double*tabD);
// extern  void getSpectrum2(int wPol, double M, char*n1,char*n2,int outP, double *tab);

extern aChannel* vSigmaCh;
extern double xInterp(double x, double * tab);
/* input parameter:
        a) x=E/tab[0]
        b) double tab[NZ] table obtainaed by calcSpectrum;
   output:
         x*dN/dx 
*/
...
extern  double  SYukawa(double a, double b);
extern  double  SHulthen(double a, double b); 
...
extern void setProfileEinasto(double alpha, double r2);
...
extern char * outNames[7];

extern int basicSpectra(double Mass,int pdgN, int outN, double * tab);
extern int spectUncert, DHad, DScale, DcNS;
extern int spectraUncertainty(double Mass, int pdgN, int outN, double * tab);
