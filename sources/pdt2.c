...
{ double x=exp(-y);
  ...
  double f1=parton_distr(pc1_,x, q_), f2=parton_distr(pc2_,x0_/x, q_);
  if(f1<0) f1=0;
  if(f2<0) f2=0;
...
{ 
  ...
  {
    for(int i=0;i<36;i++) if(cData[i]) {freePdtData(cData[i]); free(cData[i]); cData[i]=NULL;}
  ...
  if(cData[nc]) { if(nPROCSS>1) pthread_mutex_unlock(&lockKey);  return cData[nc];}
