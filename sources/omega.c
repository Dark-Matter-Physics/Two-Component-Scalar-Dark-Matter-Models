              ...
              {  double  brV1,wV1;
                 ...
                 {  double eps=0.01, delta= 0.0001;
                    ...
                    *(cc23->interface->gswidth)=1;
                    ...
                    if( pmass[0]+pmass[1]>=pmass[l_]) Pcm0=(pmass[0]+pmass[1])*1E-4; else
...
*/
           for(i=0;i<vgrid.n;i++)
          {
             ...
             if(_fast_)   da=gauss(s_integrand,vgrid.v[i],vgrid.v[i+1],vgrid.pow[i]);
                            ...
                            int err; da=simpson(s_integrand,vgrid.v[i],vgrid.v[i+1],eps,&err);
                            if(err) do_err=do_err||err;
                          }
             a+=da;
          }
...
static double *Y_a=NULL;
static double * ln_a=NULL;
static double *lnH_a=NULL;
double Ya(double a) 
{  if(Ntot_a==0) return NAN;
   double lna=log(a);
   if(a<1 || lna > ln_a[Ntot_a-1] ) return NAN;
   return polint3(lna,Ntot_a,ln_a,Y_a);
}  

double Za(double a) 
{   double T=Ta(a);  
    return  Ya(a)*2*M_PI*M_PI/45*pow(T*a,3)*hEff(T);
}

double ZaEq(double a) 
{   double T=Ta(a);  
//printf("a=%E  T=%E  Yeq=%E pow=%E hEff=%E \n", a,T, Yeq(T), pow(T*a,3),hEff(T));
    return  Yeq(T)*2*M_PI*M_PI/45*pow(T*a,3)*hEff(T);
}
...
{  if(Ntot_a==0) return NAN;
   double lna=log(a);
   if(a<1 || lna> ln_a[Ntot_a-1] ) return NAN;
   return polint3(lna,Ntot_a,ln_a,T_a);
}
...
static double s3Start,GammaStat;

static void Xderiv_a3_stiff(double x, double *Y, double *dYdx, double h, double*dYdxdx, double*dYdxdY )
{
  double a=exp(x);
//printf("lna=%E\n", x);  
  double z=Y[2];
...
//printf(" log(a)=%E  T=%E\n",x,T);
  ...
  {  double alpha,vSig;
     if(T==0) vSig=vSigmaI(Tend,Beps_,&alpha); else vSig=vSigmaI(T,Beps_,&alpha);
     double rho_sm=M_PI*M_PI/30*gEff(T)*pow(T,4);
     double rho_f=Y[1]/pow(a,3);
     double H=sqrt(8*M_PI/3*(rho_sm+rho_f))/MPlanck;
     double c=pow(hEff(T)*2*M_PI*M_PI/45,1./3.) ;   

     dYdx[0]=4./3.*GammaStat/H*Y[1]*c*a;   
     dYdx[1]=-GammaStat/H*Y[1];
     dYdx[2]=-1/H/pow(a,3)*vSig*(z*z-(1-alpha)*zeq*zeq-alpha*z*zeq);
//printf("lna=%E T=%e Y=%e %e %e   dYdx= %E %E %E\n", x,T, Y[0],Y[1],Y[2], dYdx[0], dYdx[1],dYdx[2]);

     double eps=0.01;
     if(dYdxdx) 
     { double dx=0.0001;
       double dY[3];
       Xderiv_a3_stiff(x+dx, Y, dY, h, NULL, NULL);
       for(int i=0;i<3;i++) dYdxdx[i]=(dY[i]-dYdx[i])/dx;
//printf("dYdxdx =%E %E %E\n", dYdxdx[0],dYdxdx[1],dYdxdx[2]);     
...
/*
       for(int i=0;i<3;i++)
       {
          if(Y[i]==0) { dYdxdY[i]=0;dYdxdY[3+i]; dYdxdY[6+i]=0;} else
          {
            double dY2[3],Y2[3];
            for(int k=0;k<3;k++) Y2[k]=Y[k];
            Y2[i]+=0.01*Y[i];
            Xderiv_a3_stiff(x, Y2, dY2, h, NULL, NULL); 
            dYdxdY[i]=100/Y[i]*(dY2[0]-dYdx[0]);     dYdxdY[3+i]=100/Y[i]*(dY2[1]-dYdx[1]) ; dYdxdY[6+i]=100/Y[i]*(dY2[2]-dYdx[2]); 
          }        
       }
return;
*/
//        double dTdY0=T/(4*Y[0])/(1+hEffLnDiff(T)/3); 

        double dzeqdY0=0;
        if(T) dzeqdY0=1/(4*a*s3)*( (Yeq(T+eps*T)-Yeq(T-eps*T))/(2*eps)*(1+hEffLnDiff(T)/3) +3*Yeq(T)); 
         
        double dHdY0= M_PI/(c*H*pow(MPlanck*a*a,2)); //ok
        
        dYdxdY[0*3+0]=  -4./3.*GammaStat*a*Y[1]/pow(H,2)*dHdY0*c;
            if(T>1E-3)  dYdxdY[0*3+0]+= 4./9.*GammaStat/H*Y[1]*hEffLnDiff(T)/(1+hEffLnDiff(T)/3)/pow(Y[0],0.75); 
        dYdxdY[0*3+1]=   4./3.*GammaStat*a*c/H*(1 -4*M_PI/3.*rho_f/pow(H*MPlanck,2)); //ok
        dYdxdY[0*3+2]=0;
        dYdxdY[1*3+0]=        GammaStat*Y[1]/pow(H,2)*dHdY0;                          // ok  
        dYdxdY[1*3+1]=       -GammaStat/H*(1 -4*M_PI/3.*rho_f/pow(H*MPlanck,2));       //ok
        dYdxdY[1*3+2]=0;  
        dYdxdY[2*3+0]= -dYdx[2]/H*dHdY0 + 1/H/pow(a,3)*vSig*(2*(1-alpha)*zeq+alpha*z)*dzeqdY0; //ok 
        dYdxdY[2*3+1]= -dYdx[2]*4*M_PI/3./a/pow(MPlanck*H*a,2);                         //ok 
        dYdxdY[2*3+2]= -1/H/pow(a,3)*vSig*(2*z-alpha*zeq);                              //ok
//printf("dYdxdY =%E %E %E %E %E %E %E %E %E\n", dYdxdY[0],dYdxdY[1],dYdxdY[2],dYdxdY[3],dYdxdY[4], dYdxdY[5],dYdxdY[6],dYdxdY[7],dYdxdY[8]);
//printf("dYdxdY_=%E %E %E %E %E %E %E %E %E\n", dYdxdY_[0],dYdxdY_[1],dYdxdY_[2],dYdxdY_[3],dYdxdY_[4], dYdxdY_[5],dYdxdY_[6],dYdxdY_[7],dYdxdY_[8]);
     }
  }
}

static void Xderiv_a2_stiff(double x, double *Y, double *dYdx, double h, double*dYdxdx, double*dYdxdY )
{
  double a=exp(x);
//printf("lna=%E\n", x);  
  double z=Y[2];
...
//printf("lna=%E T=%e Y=%e %e %e   dYdx= %E %E %E\n", x,T, Y[0],Y[1],Y[2], dYdx[0], dYdx[1],dYdx[2]);
     ...
     { double dx=0.0001;
       double dY[3];
       Xderiv_a2_stiff(x+dx, Y, dY, h, NULL, NULL);
       for(int i=0;i<2;i++) dYdxdx[i]=(dY[i]-dYdx[i])/dx;
//printf("dYdxdx =%E %E %E\n", dYdxdx[0],dYdxdx[1],dYdxdx[2]);     
     } 
     if(dYdxdY) 
     {
/*       for(int i=0;i<2;i++)
       {
          ...
          if(Y[i]==0) { dYdxdY[i]=0;dYdxdY[2+i]; } else
          {
            double dY2[3],Y2[3];
            for(int k=0;k<2;k++) Y2[k]=Y[k];
            Y2[i]+=0.01*Y[i];
            Xderiv_a2_stiff(x, Y2, dY2, h, NULL, NULL); 
            dYdxdY[i]=100/Y[i]*(dY2[0]-dYdx[0]);     dYdxdY[2+i]=100/Y[i]*(dY2[1]-dYdx[1]) ; 
          }        
       }
return; 
*/
        ...
        dYdxdY[0*2+0]=  -4./3.*GammaStat*a*Y[1]/pow(H,2)*dHdY0*c ; 
           if(T>1E-3)  dYdxdY[0*3+0]+= 4./9.*GammaStat/H*Y[1]*hEffLnDiff(T)/(1+hEffLnDiff(T)/3)/pow(Y[0],0.75);
        dYdxdY[0*2+1]=   4./3.*GammaStat*a*c/H*(1 -4*M_PI/3.*rho_f/pow(H*MPlanck,2)); //ok
//        dYdxdY[0*3+2]=0;
        dYdxdY[1*2+0]=        GammaStat*Y[1]/pow(H,2)*dHdY0;                          // ok  
        dYdxdY[1*2+1]=       -GammaStat/H*(1 -4*M_PI/3.*rho_f/pow(H*MPlanck,2));       //ok
//        dYdxdY[1*3+2]=0;  
//        dYdxdY[2*3+0]= -dYdx[2]/H*dHdY0 + 1/H/pow(a,3)*vSig*(2*(1-alpha)*zeq+alpha*z)*dzeqdY0; //ok 
//        dYdxdY[2*3+1]= -dYdx[2]*4*M_PI/3./a/pow(MPlanck*H*a,2);                         //ok 
//        dYdxdY[2*3+2]= -1/H/pow(a,3)*vSig*(2*z-alpha*zeq);                              //ok
...
static double isWimpFactor=1;

static void Xderiv_a1_stiff(double x, double *Y, double *dYdx, double h, double*dYdxdx, double*dYdxdY )
{
  double a=exp(x);
  double z=Y[0];
  if(Y[0]<0) Y[0]=0;

  double T=polint3(x,Ntot_a,ln_a,T_a)  , H=Ha(a);
  
  MassCut=2*Mcdm -T*log(Beps_);
  double  zeq=Yeq(T)*2*M_PI*M_PI/45.*hEff(T)*pow(T*a,3);
  if(T<=0) zeq=0; 
  {  double alpha,vSig;
     if(T<=0) {vSig=vSigmaI(Tend,Beps_,&alpha);zeq=0;}  else vSig=vSigmaI(T,Beps_,&alpha);
     vSig*=isWimpFactor;
...
//printf(" x=%E  T=%E  H=%e  vSig=%E z=%E zeq=%E (Y=%.2E s=%.2E)   dYdx[0]=%e\n",x,T,H,vSig,z, zeq,   Yeq(T), 2*M_PI*M_PI/45.*hEff(T)*pow(T,3),  dYdx[0]);
     ...
     { double dx=0.0001;
       double dY[1];
       Xderiv_a1_stiff(x+dx, Y, dY, h, NULL, NULL);
       dYdxdx[0]=(dY[0]-dYdx[0])/dx;
     } 
     if(dYdxdY) dYdxdY[0]=  -1/H/pow(a,3)*vSig*(2*z-alpha*zeq);
  }
}

static void Xderiv_a1_odeint(double x, double *Y, double *dYdx)
{
   Xderiv_a1_stiff(x, Y, dYdx, 0.01, NULL, NULL );  
}


double dEq_a(double a)
{
   double T=Ta(a),zeq=Yeq(T)*2*M_PI*M_PI/45.*hEff(T)*pow(T*a,3);
   double dlna=-0.01;
   double a_=a*exp(dlna), T_=Ta(a_),zeq_=Yeq(T_)*2*M_PI*M_PI/45.*hEff(T_)*pow(T_*a_,3);
   double alpha;
   double vSig=vSigmaI(T,Beps_,&alpha);  
   return  (zeq_-zeq)*Ha(a)*a*a*a/( zeq*zeq*dlna*(2-alpha)*vSig);   
}


int RHFlag=1;

double darkOmegaInflDecay(double H0, double Gamma, double Beps,double *aa,  double *Trh, int * isWimp,   int *err)
{ _fast_=0;
  QaddressFlag=1;
  int i;
  if(err) *err=0;
  ...
  vSigmaGrid.pow=0;
  double alpha;  
//  vSigmaI(Tr, Beps,&alpha);
//  s3Start=pow(2*M_PI*M_PI/45*hEff(Tr),1./3.)*Tr;
  double a=1, lna=0;
  int na50=50;
  ...
  Y_a=  (double *)realloc(Y_a, na50*sizeof(double));
  ln_a[0]=0;
  T_a[0]=0;
  Y_a[0]=0;
  lnH_a[0]=log(H0);
  ...
  { if(na>=na50)
    ...
    if(na>1)  yscal[0]=0;
    int er;
    if(RHFlag) er=stiff(first,lna, lna+step, 2, Y , yscal, 1E-3, &htry, Xderiv_a2_stiff);
    else       er=stiff(first,lna, lna+step, 3, Y , yscal, 1E-3, &htry, Xderiv_a3_stiff);
    if(er)    
    { printf("problem in solving diff. equation\n");
      Ntot_a=na;
      if(aa)*aa=exp(lna);
...
//    if(step>0.1) step=0.1;  
    ...
    if(RHFlag==0) Y_a[na]=Y[2]/pow(s3*a,3);
    T_a[na]=T;     
...
//  printf("Ntot=%d step=%e\n",Ntot_a,step);
  ...
  {  double Zend[2];
     ...
     {   
       if(kk && k==0) isWimpFactor=1.01; else isWimpFactor=1;
       for(na=0;na<Ntot_a;na++) Y_a[na]=NAN;
       first=1; 
       double dz;
       for(na=Ntot_a-1;na>0;na--)
       {
          ...
          double alpha,vSig;
          vSig=vSigmaI(T,Beps_,&alpha)*isWimpFactor;
          double H=exp(lnH_a[na]);
//printf("a=%E T=%E step=%E\n",a, T, H*a*a*a/(zeq*(2-alpha)*vSig));        
          if( H*a*a*a > 1E-3*zeq*(2-alpha)*vSig)  continue;        
          double dlna=-0.01;
          double T_=polint3(lna+dlna,Ntot_a,ln_a,T_a);
          double zeq_=Yeq(T_)*2*M_PI*M_PI/45*hEff(T_)*pow(T*a*exp(dlna),3);
          dz=-(zeq_-zeq)/dlna*H*a*a*a/(zeq*(2-alpha)*vSig);     
//printf("a=%E T=%E  da/zeq=%E\n",a,T, dz/zeq);   
//       if( H*a*a*a > 1E-3*zeq*(2-alpha)*vSig)  continue;              
       ...
       { double T=T_a[na]; 
         ...
         Y_a[na]=Yeq(T)*(1+dz/zt);  
         na++;
       } else  {Y_a[0]=0; na=1;} 
       first=1;

       htry=ln_a[na+1]-ln_a[na]; 
       for(;na<Ntot_a ;na++)
       { 
          if(odeint(&zt,1 ,ln_a[na-1] , ln_a[na] , 1.E-3,  (ln_a[na-1] - ln_a[na])/2 , Xderiv_a1_odeint))
          { printf("problem in solving diff. equation\n");
          ...
          double T=T_a[na];
          a=exp(ln_a[na]);
          Y_a[na]=zt/( 2*M_PI*M_PI/45*hEff(T)*pow(T*a,3));
       }
       Zend[k]=Y_a[Ntot_a-1]; 
     }
  ...
  dmAsymm=0;
  if(FError) { if(err) *err=8|simpson_err ;  goto theEnd;}
  if(err) *err=simpson_err;
  theEnd:
  fracCDM[wimp]=1;
//printf("THE END\n");  
  omega= 2.742E8*Mcdm*Y_a[Ntot_a-1];
  Mcdm=McdmMem;
  return omega;
}

int InflInfo( double *Tmax, double *Trh, double *Teq, double * Tfo)
{ 
  if(Ntot_a==0) return 0;
  double Tm=0,Tr=0;
  
  for(int n=0;n<Ntot_a;n++)
  { if(T_a[n]>Tm) Tm=T_a[n]; 
    if(Tr==0 && exp(2*lnH_a[n])<=  2* 8*M_PI/3* M_PI*M_PI/30*gEff(T_a[n])*pow(T_a[n],4)/MPlanck/MPlanck) Tr=T_a[n];   
  }
  ...
  int ne=0;
  double Te=NAN;
  for(int n=Ntot_a-1;n>0;n--) if(isfinite(Yeq(T_a[n])))
  {
    if(!isfinite(Te))
    {
       if(fabs(Y_a[n]-Yeq(T_a[n])) < 0.01*Yeq(T_a[n]))
       {  ne=n;
          Te=T_a[n];
       }
    }   
    ...
    {
       if(ln_a[n]+1 < ln_a[ne]) break;
       if(fabs(Y_a[n]-Yeq(T_a[n])) > 0.01*Yeq(T_a[n])) Te=NAN;
    }     
  }
  if(Teq) *Teq=Te;
  
  double Tf=NAN;
  if(isfinite(Te))for(int n=ne;n<Ntot_a;n++)
  { 
    if(Y_a[n]> 2.5*Yeq(T_a[n]))
    {
      Tf=T_a[n];
      break;
    }  
              ...
              {  double  brV1,wV1;
                 ...
                 {   double eps=0.01, delta= 0.0001;
                    ...
                    *(cc23->interface->gswidth)=0;
                    ...
                    if( pmass[0]+pmass[1]>=pmass[l_]) Pcm0=1E-4*(pmass[0]+pmass[1]); else
...
//  Y[1]=-Yeq2(TR)+Y2R;
  ...
  { double h=0.01*Tstart*(1-1/step);
    ...
    { double T2=T/step;
      ...
      if(Yscal[1]==0) Yscal[1]=1E-5*Yeq2(T);
      double Ymax=Yscal[0];
...
static double dSdC( double C, void* argVoid)
{
  cs22IntSt* arg=(cs22IntSt*) argVoid;
  REAL pvect[16];
...
static double u_integrand_vs( double u,void*argVoid )
{
...
//  if(vP==3) { double mm=arg.m[3]; arg.m[3]=arg.m[2]; arg.m[2]=mm; }  // ??????
  ...
  double pcm0,pcm1;
  if(sqrtS1 >= arg.m[l_]+mMinVirt) pcm0=1E-4*sqrtS1; else  pcm0 = decayPcm( arg.m[l_]+mMinVirt, arg.m[0], arg.m[1]);
...
//        argI.sqrtSmin= cL->mMin+dMir;
        ...
        { REAL m0=*cL->mAddress[0], m1=*cL->mAddress[1];
          ...
          { REAL m01=m0+m1;
            REAL m23=(*cL->mAddress[2]) +(*cL->mAddress[3]);
            if(m01>m23) argI.sqrtSmin=m01+dMir; else argI.sqrtSmin=m23+dMir;
          }     
        }
        argI.cc=cL->cc;
        argI.cL=cL;
...
static double vSigmaNstatLogT(double logT, void* argVoid)
{ 
  vSigmaNArg* arg=(vSigmaNArg*) argVoid;
  double T=exp(logT); return vSigmaNstat(T,arg)/(T*sqrt(T));
}
...
static void* interpolateCycle(void*unused)
{
  for(;;)
  {
     if(dynamic_cs_mutex) {pthread_mutex_lock(dynamic_cs_mutex);}
...
void setFastBeps( int Fast, double Beps) { _fast_=(Fast>0); _beps_=Beps;}
