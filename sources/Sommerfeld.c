...
double SYukawa(double a, double b) 
{  aa=a, bb=b;
   double xstart=0; 
...
double SHulthen(double a, double b)  // 1704.02149 Eq.25
{
  double a_=1/(2*a);
  double c_=12*a/b/M_PI/M_PI;
  double theta_=c_-a_*a_*c_*c_;
  double l=2*M_PI*a_*c_;
  double res; 
  
  if(l<100)
  {
     double cstheta;
     if(theta_>=0) cstheta= cosl(2*M_PI*sqrt(theta_)) ; 
             else  cstheta=coshl(2*M_PI*sqrt(-theta_));    
     res =  M_PI/a_*sinh(l)/(coshl(l) - cstheta); 
  }
  else 
  {  if(theta_>=0) res=M_PI/a_;
     else res= M_PI/a_/(1- exp(2*M_PI*sqrt(-theta_) -l));  
  }
