...
static double   ZhaoRc     =   8.1;
static double   ZhaoAlpha =   1;
static double   ZhaoGam   =   1;
static double   ZhaoBeta  =   3;

double hProfileZhao(double r)
{
   double rhomax,rho,a1,a2,a3; 
   a1=pow((Rsun/r),ZhaoGam);
   a2=1+pow(Rsun/ZhaoRc,ZhaoAlpha);
   a3=1+pow(r/ZhaoRc,ZhaoAlpha);
   rho = a1 * pow(a2/a3,(ZhaoBeta-ZhaoGam)/ZhaoAlpha );
...
static double alphaEin=2.33, r2Ein=10.7;

double hProfileEinasto(double r)
{ 
   return  exp(-2*(pow(r/r2Ein,alphaEin)- pow(Rsun/r2Ein,alphaEin))/alphaEin);
} 

void setProfileEinasto(double alpha, double r2)
{  alphaEin=alpha;
    r2Ein=r2;
