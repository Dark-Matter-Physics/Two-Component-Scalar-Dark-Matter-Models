...
static double  s_integrand(double x)
{  double r,q;
   REAL pcmIn;
   int  err;
   double pp=3; // it was 1.5 in previous versions
   
   REAL  s=sMin*Pow(1+ x*( Pow(sMax/sMin,1-pp) -1 ),1/(1-pp));
   double x0=s/sMax;
...
//   double pp=1.5;
   ...
   if(npole34==0 && npole35==0)  { cs45=0.99999*(2*x[3]-1); J45=2;}  else  // 0.9999 appears in this version. Infrared problem???  
   {  
   ...
   r=  kinematic_23(pcmIn,i3,M45, cs ,cs45,M_PI*x[4],pmass, pvect)*4*M_PI*(M45_max-M45_min)*J45*cs_/pcmIn;
   double qr,qf,q0;
...
//printf("vegas_int  ti=%E dti=%E  NN=%d \n",ti[k], dti[k],*NN);
...
double vegas_cycle23(numout*cc, double Pcm, double ptMin,  double eps, double aeps,int maxStep, int*NN, double fact,double *dI)
{ 
    passParameters(cc);
    vegasGrid *vegPtr=vegas_init(5,veg3_intergrand,50);
    MET=0;
    sMax=4*Pcm*Pcm;  
    pTmin_=ptMin;
    nsub22=1;
    ...
    M45_max=2*Pcm;
    i3=2,i4=3;i5=4;
    for(int i=0;i<5;i++) cc->interface->pinf(nsub22,i+1,pmass+i,NULL);
    return  vegas_cycle(vegPtr, eps, aeps, maxStep, NN, fact,dI);
}
...
//printf("( %.2f%%)\n",100*dI/metArr_[0]);
                ...
                {  int err;
                   tmp=simpson(s_integrand,0.,1.,1.E-3,&err); if(err) printf("error in simpson cs22 line 748\n"); 
                }          
//                if(!met && wrt)printf("cs=%E \n", tmp);  
        ...
        {  double m3q;
           ...
           int NN=100000;
           if(fabs(sigma_tot)*eps>aEps) aEps=sigma_tot*eps;          
           tmp=vegas_cycle(vegPtr,eps, aEps, 10,&NN, 1.5,&dI);           
           if(met)
           {       for(i=0;i<METDIM;i++) {metArr_[i]=0; dmetArr_[i]=0;}
                  fillArr=1;  
//                  printf("tmp=%E dI=%e\n", tmp,dI);
...
{ 
  ...
  char oldPDF[100]={""};
  if(strcmp(pdfName,"cteq66"))
  {  strcpy(oldPDF,pdfName);
     setPDT("cteq66");
  } else strcpy(oldPDF,"cteq66");
  

  METmin_=METmin;
  for(i=0;i<METDIM;i++) sum[i]=0;
  
  for(int k=1;k<=Ncdm;k++) if(CDM[k])
  {   
     ret=hColliderStat(4000, 1, NF, 0, 0, CDM[k],antiParticle(CDM[k]),METmin/(1+pt2etRange), 1, 0);
     for(i=0;i<METDIM;i++) sum[i]+=metArr[i];
     if(strcmp(CDM[k],antiParticle(CDM[k])))
     { ret=hColliderStat(4000, 1, NF, 0, 0, CDM[k],CDM[k],METmin/(1+pt2etRange), 1, 0); 
