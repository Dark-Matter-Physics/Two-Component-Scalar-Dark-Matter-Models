...
//printf("bath=%s\n", bath);
  ...
  sprintf(command,"cd %s;"
           " %s/bin/s_calchep -blind \"{%s{allBath,allBath->feeblDm,1*x{%s{%s{{{[[{0\" >/dev/null;"
           " if(test $? -eq 0) then mv results/list_prc.txt %s; fi",
            compDir,calchepDir, UG,bath+1,feebleDm+1,libDir);
  system(command); 
  free(command);
//printf("allBath=%s\n", bath+1);  
...
static double NabDmDmInt(double x, void * argVoid)
{  int err;
 
   frin22Par* arg=(frin22Par*) argVoid;
   if(x==0 || x==1) return 0;
...
static double TdYdTfreezeInLn(double lnT, void*argVoid)
{   
  frin22Par*arg=(frin22Par*) argVoid;
  double T=exp(lnT);   return  TdYdTfreezeIn(T, arg);
}
