	...
	void (*derivs)(double,double*,double*,double,double*,double*))
{
    ...
    {    
      ...
      *x=xsav+h;
      ...
      {  double erri;  
         if(yscal) 
         {  if(yscal[i]>0) erri=fabs(yerr[i]/yscal[i]); else
                           erri=fabs(yerr[i])/(fabs(y[i])+TINY);
         }
         else              erri=fabs(yerr[i])/(fabs(y[i])+TINY);
         if(errmax<erri) errmax=erri;
      }         
      errmax /= eps;
//printf("             y= %E %E %E  \n",jtry,  y[0],y[1],y[2]);
//printf(" yscale         %E %e %E  \n",yscal[0],yscal[1],yscal[2]);
//printf("jtry=%d   yerr= %E %E %E errmax=%E\n",jtry,  yerr[0],yerr[1],yerr[2],errmax);
